...
func (h *OAuthHandler) signInExisting(ctx echo.Context, userID uuid.UUID) error {
	user, err := h.users.GetUserByID(ctx.Request().Context(), userID)
	if err != nil {
	...
	if err := h.sessions.CreateSession(ctx.Request().Context(), session); err != nil {
...
func (h *OAuthHandler) CompleteSignUp(ctx echo.Context) error {
	...
	if exists, err := h.users.ExistsUserByEmail(ctx.Request().Context(), string(req.Email)); err != nil {
		return internalError(ctx, "Failed to check email uniqueness")
	} else if exists {
		return conflict(ctx, "User with this email already exists")
	}
	if exists, err := h.users.ExistsUserByUsername(ctx.Request().Context(), req.Username); err != nil {
	...
	if err := h.users.CreateUser(ctx.Request().Context(), user); err != nil {
