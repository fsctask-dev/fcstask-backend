...
// GET /api/courses/:courseId/deadlines/:deadlineId
func GetCourseDeadlineHandler(c echo.Context) error {
	courseID := c.Param("courseId")
	deadlineID := c.Param("deadlineId")

	if courseID == "" || deadlineID == "" {
		return c.JSON(http.StatusBadRequest, map[string]string{
			"error": "course ID and deadline ID are required",
		})
	}

	courseMu.RLock()
	_, exists := courseDB[courseID]
	courseMu.RUnlock()
	...
	board, ok := boardData[courseID]
	if !ok {
		return c.JSON(http.StatusNotFound, map[string]string{
			"error": "board not found for this course",
		})
	}

	for _, group := range board.Groups {
		for _, deadline := range group.Deadlines {
