...
type IOAuthIdentityRepo interface {
	...
	GetProviderForUserID(ctx context.Context, userID uuid.UUID, provider string) (*model.OAuthIdentity, error)
	GetByEmailRegistrationID(ctx context.Context, emailRegistrationID uuid.UUID) (*model.OAuthIdentity, error)
...
func (r *OAuthIdentityRepository) GetProviderForUserID(ctx context.Context, userID uuid.UUID, provider string) (*model.OAuthIdentity, error) {
	var identity *model.OAuthIdentity
	err := r.db.WithContext(ctx).
...
func (r *OAuthIdentityRepository) GetByEmailRegistrationID(ctx context.Context, EmailRegistrationID uuid.UUID) (*model.OAuthIdentity, error) {
	var identity model.OAuthIdentity
	err := r.db.WithContext(ctx).
		Where("user_registration_id = ?", EmailRegistrationID).
		Order("created_at ASC").
		Find(&identity).Error
	if err != nil {
		return nil, err
