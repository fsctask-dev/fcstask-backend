...
func TestPasswordReset_Request_Success(t *testing.T) {
	...
	env.users.On("GetByEmail", mock.Anything, "ivan@example.com").Return(user, nil)
...
func TestPasswordReset_Request_UserNotFound(t *testing.T) {
	env := newPwResetEnv(t)
	env.users.On("GetByEmail", mock.Anything, "ghost@example.com").Return(nil, gorm.ErrRecordNotFound)
...
func TestPasswordReset_Resend_Success(t *testing.T) {
	...
	env.users.On("GetByID", mock.Anything, userID).Return(&models.User{ID: userID, Email: "ivan@example.com"}, nil)
...
func TestPasswordReset_Confirm_Success_InvalidatesSessions(t *testing.T) {
	...
	env.users.On("Update", mock.Anything, mock.MatchedBy(func(u *models.User) bool {
		return u.PasswordHash != "" && u.PasswordHash != "old"
	})).Return(nil)
	env.sess.On("DeleteByUserID", mock.Anything, userID).Return(nil)
